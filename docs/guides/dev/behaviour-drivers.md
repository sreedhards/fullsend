...
Uses `fullsend github setup <org>/test-repo --vendor --direct --skip-app-setup --runtime dummy` with inference flags from `E2E_GCP_PROJECT_ID` and `E2E_GCP_WIF_PROVIDER`. Pool orgs must already have shared GitHub Apps and mint enrollment; the driver does not run `fullsend admin install`.
