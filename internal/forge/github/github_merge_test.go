...
func TestMergeChangeProposal_409PersistsAfterRetries(t *testing.T) {
	...
	assert.Equal(t, int32(3), mergeAttempts.Load(), "should have tried exactly maxAttempts times")
}

func TestMergeChangeProposal_UpdateBranchFailsReturnsError(t *testing.T) {
	var mergeAttempts atomic.Int32
	...
	err := client.MergeChangeProposal(context.Background(), "org", "repo", 7)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "update branch")
	assert.Equal(t, int32(1), mergeAttempts.Load(), "should not retry after update-branch failure")
}
